...
TEST( Test_AllocatedMemTop, CanConstruct )
{
	...
	// Act
	tut* p_ret = tut::emplace_on_mem<char>( buff, nullptr, alpha::concurrent::internal::mem_type::OVER_BIG_MEM, true );
...
TEST( Test_AllocatedMemTop, DoGetStructureAddr )
{
	// Arrange
	unsigned char buff[sizeof( tut ) * 10];
	tut*          p_sut = tut::emplace_on_mem<char>( buff, nullptr, alpha::concurrent::internal::mem_type::OVER_BIG_MEM, true );
...
TEST( Test_AllocatedMemTop, DoLoadAllocationInfo )
{
	...
	// Act
	auto ret = p_sut->load_allocation_info<char>();
